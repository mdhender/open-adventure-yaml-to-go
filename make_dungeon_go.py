...
# This file generates Go files from the open-adventure dungeon file.
# It consumes a YAML description of the dungeon and outputs a
# defines.go and dungeon.go pair of Go code files.
#
# This script was created from open_adventure's make_dungeon.py file.
# That original script is:
...
H_NAME = "defines.go"
C_NAME = "dungeon.go"
H_TEMPLATE_PATH = "templates/defines.go.tpl"
C_TEMPLATE_PATH = "templates/dungeon.go.tpl"

DONOTEDIT_COMMENT = "// Code generated by \"adventure-yaml\" on YYYY-MM-DDTHH24:MI:SSZ; DO NOT EDIT.\n\n"
...
def get_string_group(strings):
    ...
    if strings == []:
        ...
        strs = "[]string {" + ", ".join([make_c_string(s) for s in strings]) + "}"
    n = len(strings)
...
"""
    arb_str = ""
    for (i, item) in enumerate(arb):
        arb_str += template.format(i, item[0], make_c_string(item[1]))
...
"""
    ...
    for (i, motion) in enumerate(motions):
        ...
        mot_str += template.format(i, motion[0], words_str)
        global ignore
...
"""
    ...
    for (i, action) in enumerate(actions):
        ...
        act_str += template.format(i, action[0], words_str, message, noaction)
        global ignore
        if contents.get("oldstyle", True) == False:
            for word in contents["words"]:
                if len(word) == 1:
